...
func NewRouter(cfg config.Config, authHandler *handler.AuthHandler, sessionHandler *handler.SessionHandler, realtimeHandler *handler.RealtimeHandler) *gin.Engine {
	...
	protected.POST("/sessions/:id/recordings", sessionHandler.UploadRecording)
	protected.GET("/sessions/:id/recordings", sessionHandler.ListRecordings)
	protected.GET("/sessions/:id/recordings/:recordingId/file", sessionHandler.DownloadRecording)
